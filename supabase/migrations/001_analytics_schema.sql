...
DROP TABLE IF EXISTS performance_summary CASCADE;
DROP TABLE IF EXISTS learning_sessions   CASCADE;
DROP TABLE IF EXISTS quiz_questions      CASCADE;
DROP TABLE IF EXISTS quiz_attempts       CASCADE;
DROP TABLE IF EXISTS mastery_levels      CASCADE;

-- Drop trigger function so CREATE OR REPLACE starts clean
DROP FUNCTION IF EXISTS compute_mastery_flags CASCADE;
...
CREATE INDEX IF NOT EXISTS idx_mastery_user            ON mastery_levels(user_id);
CREATE INDEX IF NOT EXISTS idx_mastery_subject_topic   ON mastery_levels(subject, topic);
CREATE INDEX IF NOT EXISTS idx_mastery_struggling      ON mastery_levels(user_id) WHERE is_struggling = TRUE;
CREATE INDEX IF NOT EXISTS idx_mastery_needs_revision  ON mastery_levels(user_id) WHERE needs_revision = TRUE;
...
CREATE INDEX IF NOT EXISTS idx_quiz_created     ON quiz_attempts(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_quiz_topic       ON quiz_attempts(subject, topic);
...
CREATE INDEX IF NOT EXISTS idx_qq_attempt   ON quiz_questions(attempt_id);
CREATE INDEX IF NOT EXISTS idx_qq_user      ON quiz_questions(user_id);
CREATE INDEX IF NOT EXISTS idx_qq_topic_tag ON quiz_questions(topic_tag);
CREATE INDEX IF NOT EXISTS idx_qq_incorrect ON quiz_questions(user_id) WHERE is_correct = FALSE;
...
CREATE INDEX IF NOT EXISTS idx_sessions_date    ON learning_sessions(started_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_perf_summary_user   ON performance_summary(user_id);
...
CREATE INDEX IF NOT EXISTS idx_risk_active        ON risk_flags(user_id) WHERE is_resolved = FALSE;
CREATE INDEX IF NOT EXISTS idx_risk_level         ON risk_flags(risk_level);
...
BEGIN
    IF EXISTS (
        ...
        CREATE POLICY "own_mastery"           ON mastery_levels      FOR ALL
            USING (auth.uid() = (SELECT auth_user_id FROM users WHERE user_id = mastery_levels.user_id));
        CREATE POLICY "own_quiz_attempts"     ON quiz_attempts       FOR ALL
            USING (auth.uid() = (SELECT auth_user_id FROM users WHERE user_id = quiz_attempts.user_id));
        CREATE POLICY "own_quiz_questions"    ON quiz_questions      FOR ALL
            USING (auth.uid() = (SELECT auth_user_id FROM users WHERE user_id = quiz_questions.user_id));
        CREATE POLICY "own_sessions"          ON learning_sessions   FOR ALL
            USING (auth.uid() = (SELECT auth_user_id FROM users WHERE user_id = learning_sessions.user_id));
        CREATE POLICY "own_perf_summary"      ON performance_summary FOR ALL
            USING (auth.uid() = (SELECT auth_user_id FROM users WHERE user_id = performance_summary.user_id));
        CREATE POLICY "own_risk_flags"        ON risk_flags          FOR ALL
            USING (auth.uid() = (SELECT auth_user_id FROM users WHERE user_id = risk_flags.user_id));
    ELSE
